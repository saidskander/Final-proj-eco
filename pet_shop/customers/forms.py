...
class RegistrationForm(FlaskForm):  # 2
    ...
    """Username is taken"""  # 4
    def validatee_username(self, username):  # 4
        register_user = CustomerUser.query.filter_by(username=username.data).first()  # 4
        if register_user:  # 4
            raise ValidationError('Username is taken')  # 4

    """Email taken"""  # 4
    def validatee_email(self, email):  # 4
        register_user = CustomerUser.query.filter_by(email=email.data).first()  # 4
        if register_user:  # 4
...
class RequestResetForm(FlaskForm):
    ...
    def validate_email(self, email):
        user = CustomerUser.query.filter_by(email=email.data).first()
        if user is None:
            raise ValidationError('we sent reset password to this email adress. plz check your email')
